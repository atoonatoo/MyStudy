...
2. **[[zzz/Learning|Learning]]** - `study` `project` `coding test` `trouble shooting` `set up` `development log` `practice` `etc`

3. **[[zzz/Link|Link]]** - `blog` `references` `official site` `personal site`

4. **[[zzz/Prompt|Prompt]]** - `grammer` `obsidian` `linux` `cmd` `power shell` `git` `sql` `vim` 

5. **[[Schedule]]** - `Today` `Weekend` `Challenge` `Important`

6. **[[zzz/Employment|Employment]]** - `Employment` `Interview` `Review` `Coperate Information`
