...
2. **[[99. organize/Learning|Learning]]** - `study` `project` `coding test` `trouble shooting` `set up` `development log` `practice` `etc`

3. **[[99. organize/Link|Link]]** - `blog` `references` `official site` `personal site`

4. **[[99. organize/Prompt|Prompt]]** - `grammer` `obsidian` `linux` `cmd` `power shell` `git` `sql` `vim` 

5. **[[Schedule]]** - `Today` `Weekend` `Challenge` `Important`

6. **[[99. organize/Employment|Employment]]** - `Employment` `Interview` `Review` `Coperate Information`
