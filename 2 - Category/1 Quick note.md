...
- 북스터디 - 대규모 시스템 설계 기초
	- 5장 안정해시 설계
		- 서론
			- 수평적 규모 확장을 하기 위해서는 요청, 데이터를 균등하게 분배해야한다.
			- 안정해시는 이 문제를 해결하기 위한 보편적인 기술
			- 안정해시 자체를 알아보기 앞서서 이 안정해시가 풀고자하는 문제를 먼저 알아본다.
			  
		- 해시 키 재배치(refresh) 문제
			- `hash(key)` : 키에 대해 해시 함수를 적용한 값
			- `% N` (mod N) : 해당 숫자를 `서버 개수 n`으로 나눈 나머지
			- 결과값 = i : `i`번 서버에 해당 데이터를 저장하거나 조회하겠다는 뜻
			- 서버 풀 크기가 고정 되어있을 때는 잘동작하지만, 서버가 추가되거나 기존 서버가 삭제가 되면 그 결과로, 해시 값은 변하지 않지만, 나머지 연상을 적용하여 계산한 서버 인덱스 값은 달라진다.
			- 결과적으로 장애가 발생한 서버가 죽으면, 대부분 캐시 클라이언트가 데이터가 없는 엉뚱한 서버에 접속하게 된다.
			- 그 결과 대규모 캐시 미스가 발생, `안정 해시`는 이문제를 효과적으로 해결하는 기술이다.
			  
		- 안정 해시(consistent hash)
			- 정의 : 해시 테이블 크기가 조정될 때 평균적으로 오직 `k/n`개의 키만 재배치하는 해시 기술이다.
			- 여기서 `k`는 키의 개수, `n`은 슬롯(slot)의 개수이다.
			- `해시 함수 f` 
				- 임의의 입력 값을 고정된 크기의 해시값으로 변환하는 함수
				- `f(key)` 또는 `hash(key)`로 표현함
			- `SHA-1(Secure Hash Algorithm 1)`
			- 입력의 길이 상관없이 160비트를 고정으로 출력
			- `해시 공간(hash space)`
				- SHA-1 해시 함수f로 사용한다고 가정한다.
				- 함수의 출력값 범위는 x0, x1, x2, x3 ... xn이라고 하자.
				- SHA-1의 해시 공간의 범위는 0부터 2의160승-1이다.
				- x0은 0, xn은 2의160승-1이고, x1은 xn-1까지는 그 사이의 값을 갖게 된다.
			- `해시 링(hash ring)`
				- 해시 링은 `해시 공간`을 양쪽으로 구부려 접으면 `해시 링(hash ring)`이 만들어진다.
			- 해시 서버, 해시 키
			- 서버 조회
				- 키는 해당 키의 위치로부터 시계 방향으로 링을 탐색하고 만나는 첫번 째 서버이다. 
				- 이 서버가 어떤 키가 저장되는 서버이다.
			- 서버 추가
				- 하나의 서버가 추가되면 시계 방향으로 순회된 키가 새로 추가된 서버를 만나게 된다면 그 키는 재배치를 받게 된다.
				- 나머지는 영향을 안받는다.
			- 서버 제거
				- 하나의 서버가 제거되면 키 가운데 일부분만 재배치되고, 나머지는 영향을 안받는다.
			
		- 기본 구현법과 문제점
			- MIT에서 처음 제안된 안정 해시 알고리즘 기본 절차
				- 서버와 키를 균등 분포(uniform distrivution) 해시 함수를 사용해 해시 링에 배치한다.
				- 키의 위치에서 링을 시계 방향으로 탐색하다. 만나는 최초의 서버가 키가 저장될 서버이다.
			- 문제점
				1. 서버가 추가, 제거 되는 상황을 감안하면 파티션(partition)의 크기가 균등하게 유지하는게 불가능해진다.
		...
		- 가상 노드(virtual node)
			...
			- 가상 노드는 해시 링위에 고르게 분포되고, 해시 키가 최초로 접근하게되는 가상 노드의 해당하는 실제 서버로 할당
			- trade off : 가상 노드의 개수를 늘리면 키의 분포는 균등해지지만, 표준 편차의 값은 떨어진다.
