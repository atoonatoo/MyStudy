...
---
## 1. 서론
- 키-값 저장소는 비관계형 NoSQL 데이터베이스  
- 각 값은 고유한 키에 매달려 저장됨  
- 키는 텍스트 또는 해시로 구성되며 유일해야 함  
- 키는 짧을수록 성능에 유리함  
- 값은 문자열, 리스트, 객체 등 제한 없이 저장 가능  
- 대표 시스템은 Redis, Memcached, Amazon Dynamo  
- put(key, value)로 저장  
- get(key)로 조회  
- 단순하고 빠르지만 메모리 한계 존재  
...
- 대용량 데이터 저장 가능해야 함  
- 장애 발생 시에도 빠른 응답 필요  
- 트래픽에 따라 서버 자동 증설 및 축소 가능  
- 데이터 일관성 수준은 상황에 따라 조절 가능  
- 응답 지연시간은 짧을수록 좋음  

---

## 3. 단일 서버 설계
- 메모리에 해시 테이블 구조로 구성 가능  
- 빠른 속도 제공  
- 메모리 한계로 전체 데이터를 담기 어려움  
- 자주 쓰는 데이터는 메모리에, 나머지는 디스크로 분산  
- 압축을 통해 메모리 효율 개선 가능  
- 단일 서버로는 확장성과 안정성에 한계 있음  
- 분산 키-값 저장소 설계가 필수적  

---

## 4. 분산 키-값 저장소
- 여러 서버에 데이터를 분산 저장하는 구조  
- 분산 해시 테이블이라고도 함  
- CAP 정리를 고려한 설계가 필요함  
...
## 5. CAP 정리
- 일관성(consistency): 모든 노드가 동일한 데이터 반환  
- 가용성(availability): 시스템은 항상 응답해야 함  
- 파티션 감내(partition tolerance): 네트워크 장애에도 시스템 동작  
- CAP 정리는 세 가지 중 두 가지만 동시에 만족 가능  
- CP 시스템: 일관성과 파티션 감내를 보장, 가용성은 낮음  
- AP 시스템: 가용성과 파티션 감내를 보장, 일관성은 낮음  
- CA 시스템: 이론상 가능하지만 실무에서는 불가능  
- 분산 시스템은 대부분 CP 또는 AP 선택  
...
## 3.3 데이터 파티션  
- 데이터를 작은 파티션으로 나누어 여러 서버에 저장  
- 고려 요소:  
  - 데이터를 고르게 분산할 수 있는가  
  - 노드 추가/삭제 시 데이터 이동을 최소화할 수 있는가  
- 안정 해시는 파티션 문제 해결에 효과적임  
- 규모 확장 자동화 가능  
  - 시스템 부하에 따라 서버 자동 추가/삭제 가능  
- 다양성 보장  
  - 서버 성능에 따라 가상 노드 수 조정 가능  
  - 고성능 서버에 더 많은 가상 노드 할당 가능  

---

## 3.4 데이터 다중화  
- 높은 가용성과 안정성을 위해 데이터를 N개의 서버에 다중화  
- N은 조정 가능한 파라미터  
- 해시 링 상에서 시계방향으로 순회하며 N개 노드에 사본 저장  
- 가상 노드 사용 시 같은 물리 서버가 중복될 수 있음  
  - 이 중복은 피해야 함  
- 동일 센터의 노드는 장애 시 함께 영향을 받을 수 있음  
  - 사본은 다른 데이터 센터에 분산 저장  
  - 센터 간 고속 네트워크로 연결 필요  
...
- 데이터를 다중화하면 가용성은 높아지지만 일관성은 깨질 가능성이 높아짐  
- 버저닝(versioning)과 벡터 시계(vector clock)는 이 문제를 해결하기 위한 기술임  

### 3.7.1 버저닝

- 데이터를 변경할 때마다 새로운 버전을 생성함  
- 각 버전은 변경 불가능함 (immutable)  

### 3.7.2 벡터 시계

- [서버, 버전]의 순서쌍을 데이터에 매달아 관리  
- 버전 간 선후 관계 및 충돌 여부를 판별할 수 있음  
- 예시 구조: D([S1, v1], [S2, v2], ..., [Sn, vn])  
  - D: 데이터, Si: 서버, vi: 버전 카운터  
  - Si에 쓰기 발생 시:
    - [Si, vi]가 있으면 vi 증가  
    - 없으면 [Si, 1] 추가  
...
- 단점:  
  - 클라이언트 구현이 복잡해짐  
  - 순서쌍이 급증함 → 오래된 항목 제거 필요  
  - 제거 시 버전 판별 정확도 저하 가능  

---

## 3.8 장애 처리

- 대규모 시스템에서 장애는 자주 발생함  
- 장애 감지 및 해소 전략이 중요함  

### 3.8.1 장애 감지

- 한 서버가 죽었을 때, 다른 여러 서버가 동일하게 인지할 때만 장애로 간주  

### 3.8.2 멀티캐스팅

- 모든 노드 간 멀티캐스팅으로 장애 감지 가능  
- 노드 수가 많을수록 비효율적임  
...
- 각 노드는 멤버십 리스트를 유지함  
  ...
  - 무작위 노드에 카운터 목록 전송  
  - 수신자는 카운터를 최신 상태로 갱신  
- 일정 시간 동안 카운터 미변경 시 장애로 간주  

### 3.8.4 일시적인 장애 처리 (Hinted Handoff)

- 쓰기/읽기 대상 서버가 일부 장애 상태인 경우  
- 건강한 서버를 임시로 선택해 연산 수행  
- 장애 서버 복구 시 변경사항 반영  
- 힌트를 기록하여 추후 동기화  

### 3.8.5 영구 장애 처리 (Anti-entropy)

- 노드가 장기적으로 장애 상태일 때 사본 동기화 필요  
- 머클 트리(Merkle Tree)를 사용하여 동기화 및 전송량 최소화  
- 머클 트리: 해시 트리로, 각 노드가 자식 노드들의 해시 요약을 가짐  
- 효율적이고 안전한 검증 구조 제공  

### 3.8.6 데이터 센터 장애 처리

- 정전, 자연재해 등으로 인한 전체 센터 장애 가능성 존재  
- 다른 데이터 센터로 다중화 필요  
- 사용자는 다른 센터의 데이터를 통해 서비스 지속 가능  

---

## 3.9 시스템 아키텍처 다이어그램

- 클라이언트는 get / put API를 통해 시스템과 통신  
- 중재자(coordinator)는 클라이언트 요청을 프록시함  
- 노드는 해시 링에 분산되어 존재함  
- 노드 자동 추가/삭제 가능  
- 데이터는 다중화되어 저장됨  
- SPOF(단일 장애점)는 없음  
...
## 3.10 쓰기 경로 (Write Path)

- 클라이언트의 쓰기 요청 흐름  
1. 요청이 커밋 로그(commit log)에 기록됨  
2. 메모리 캐시에 데이터 기록  
3. 캐시가 가득 차거나 임계치 도달 시, SSTable에 디스크 기록  
- SSTable: Sorted-String Table, 정렬된 키-값 테이블 구조  

---

## 3.11 읽기 경로 (Read Path)

- 메모리 캐시에 먼저 데이터 존재 여부 확인  
- 있으면 바로 반환  
- 없으면 디스크에서 조회 → SSTable 접근 필요  
- 효율적인 탐색 위해 블룸 필터 사용  
- 전체 흐름  
  1. 메모리 캐시 확인  
  2. 블룸 필터로 SSTable 위치 확인  
  3. SSTable에서 데이터 조회  
  4. 클라이언트에게 반환  
