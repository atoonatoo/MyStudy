...
- 6장 키-값 저장소 설계
	- 0. 서론
		- 키-값 데이터베이스라고도 불리는 `비 관계형 데이터베이스`이다.
		...
		- 예시
			...
			- 해시 키 : 253DDEC4
		- 키-값 쌍에서의 값은 문자열일 수도 있고, `List` 일수도 있고 `객체(object)`일수도 있다.
		...
		- get(key) : 인잟 주어진 키에 매달린 값을 꺼낸다.
		  
	- 1. 문제 이해 및 설계 범위 확정
		완벽한 설계란 없다는걸 감안하고 명심하도록 하자. 읽기, 쓰기, 메모리 사용량의 어떤 균형을 찾고, 데이터의 일관성 가용성 사이에 trade off를 감안한 설계를 만들었다면 도움이 되는 내용이다.
	...
	- 2. 단일 서버 키-값 저장소
		단일 서버로 저장소를 설계하려면 키-값 쌍 전부를 메모리에 해시 테이블로 작성하면 되는 쉬운 난이도이다. 하지만 이 방법은 빠른 속도는 보장하지만, 모든 데이터를 메모리에 두는 것이 불가능하다는 단점이 있다. 이 문제의 해결법으로 `데이터 압축(compression)`, `자주 쓰이는 데이터만 메모리에 두고 나머지는 디스크에 저장` 하는 방법이 있다.
		하지만 이 방법으로도 단일 서버로는 한계가 명확하기 때문에 많은 데이터를 저장하려면 필연적으로 `분산 키-값 저장소(distibuted key-value store)`을 만들 필요가 있다.
	...
	- 3.1 CAP 정리
			`데이터 일관성(consistency)`, `가용성(availability)`, `파티션 감내(partition tolerance theorem)` 이 세 사지 요구사항을 동시에 만족 하는 분산 시스템을 설계하는 것은 불가능하다는 정리이다.
		- 데이터 일관성(consistency)
		...
		- 파티션 감내(partition tolerance theorem)
			...
			CAP 정리는 이들 가운데에 어떤 2가지를 충족하려면 나머지 하나는 반드시 희생되어야 한다는 뜻이다.
			
		- 세가지 요구사항 중 2가지만 만족하는 분류
			- CP 시스템
				- 일관성, 파티션 감내를 지원하는 키-값 저장소. 가용성을 희생한다.
			- AP 시스템
				- 가용성과 파티션 감내를 지원하는 키-값 저장소. 일관성을 희생한다.
			- CA 시스템
				- 일관성과 가용성을 지원하는 키-값 저장소. 파티션 감내를 지원하지 않는다.
				- 그러나 통상 내트워크 장애는 피할 수 없는 일로 여겨진다.
				- 분산 시스템은 파티션 감내는 필수적인 요구사항으로 실무에는 CA 시스템은 존재하지 않는다.
