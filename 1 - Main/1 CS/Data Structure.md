...
- **효율적인 데이터 처리와 문제 해결을 하기 위해서**
- 자료 구조는 데이터를 어떻게 저장하고 관리할지를 정의
- 자료 구조는 특정 작업을 효율 적으로 처리 할 수 있도록 설계
...
- **데이터를 일직선으로 줄줄이 정렬하는 데이터 구조**
- 데이터의 추가나, 삭제는 쉽다.
- 원하는 데이터에 접근하는 시간은 오래걸린다.
- **ArrayList - static Array**  |  **List - Dynamic Array**
- Array size를 미리 지정하는 정적인 어레이와 달리 데이터가 들어 올떄마다 동적으로 메모리를 할당하는 자료 구조
- 용어
	- head - 연결 리스트의 시작점
...
- List의 대표적인 종류
	...
	- Circular Linked List
		- 마지막 노드가 첫번쨰 노드를 가르켜서 계속 원형으로 회전이 가능하다.
	  
- 구분법
	- 리스트의 저장 공간 Node가 있다.
	- Node안에는 데이터필드, 링크라는 변수가 있다.
	- 데이터 필드 : 데이터 저장 공간
	- 링크 : 다음 노드가 어디 있는지 알려주는 변수
	- **링크의 생김새**를 보고 구분할 수 있다.
	- 노드당 다음 노드를 알려주는 링크가 1개라면 Singly Linked List
...
- Array vs List
	...
	- input size를 모를 때, 메모리 할당 효율 ? - List
		- 다만 리스트의 특성상 Node와 Link는 4byte 용량을 가진다.
		- 리스트가 1억개라면..? 이라는 가정하에 용량에 따라 고려해봐야한다.
		  
	- 중간 데이터 추가, 삭제 효율성 - List
		- 1 ~ 100 중 50을 지우고싶다?
		- List
			- 49, 51의 노드의 연결만 바꿔주면 50을 쉽게 지울 수 있다.
			- 50으로 연결하는 링크가 사라졌기 때문에
		- Array
			- 50을 지우면 51부터 100까지 배열을 한칸씩 다 땡겨야한다.
		
	- 상황에 따라 가장 최적의 자료 구조를 사용 해야 한다. 
##### **스택 (Stack)**  
